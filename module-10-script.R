...
str(climate_data)
head(climate_data)
# The main column we're interested in is PPTAVG_SITE (average precipitation at the sight)

# Using sf functions for geospatial data manipulation ----
### Separate geometry using st_geometry() this keeps only the geometry of the file
se_states_geom <- st_geometry(se_states)

### Separate attributes using st_set_geometry(NULL) this keeps only attributes 
se_states %>%
...
cape_fear_gages %>%
  left_join(climate_data) -> cf_gages_climate

## Plot gages colored by average precipitiation of site or gage
ggplot()+
  geom_sf(data = cape_fear_basin) +
  geom_sf(data = cf_gages_climate, aes(color = PPTAVG_SITE))+
  scale_color_viridis_c()+
  labs(color = "Avg annual precip. (cm)")+
  theme_bw()+
